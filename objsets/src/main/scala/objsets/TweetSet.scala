...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    def max(left: Tweet, right: Tweet) = {
    ...
    else if(left.isEmpty) max(elem, r)
    else if(right.isEmpty) max(elem, l)
    else max(elem, max(l, r))
  }
...
object GoogleVsApple {
  ...
  def filter(list: List[String]): Tweet => Boolean = {
    t => contains(t.text, list)
  }

  def contains(text: String, list: List[String]): Boolean = {
    if (list.isEmpty) false
    else if (text.contains(list.head)) true
    else contains(text, list.tail)
  }
